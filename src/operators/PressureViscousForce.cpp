...
void PressureViscousForce::apply(SolverState& state)
{
    auto& particles      = state.particles;
    auto& neighbors      = state.neighbors;
    auto& fluid_indices  = state.fluid_indices;
    auto& accel          = state.accel;
    auto& kernel         = state.kernel;

    const double h   = state.h;
    const double mu  = state.mu;
    const double Lx  = state.Lx;
    const double Ly  = state.Ly;
...
#pragma omp parallel for schedule(static)
    for (int idx = 0; idx < (int)fluid_indices.size(); ++idx)
    {
        ...
        for (int j : neighbors[i])
        {
            ...
            const double vx_j = (pj.type == 1 && pj.vf) ? (*pj.vf)[0] : pj.v[0];
            const double vy_j = (pj.type == 1 && pj.vf) ? (*pj.vf)[1] : pj.v[1];
            ...
            // --- pressure ---
            const double p_fac =
                V_ij_sqr * (rho_j*p_i + rho_i*p_j) / (rho_i + rho_j);
        ...
        accel[i][0] = fx / pi.m + state.b_eff[0];
        accel[i][1] = fy / pi.m + state.b_eff[1];
    }
}
